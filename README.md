...
- **🎨 界面友好** - 现代化设计，操作简单直观
- **🌍 多语言支持** - 支持中文 / English / Español / Français
- **🌙 护眼模式** - 支持日间/夜间主题切换
...
| 工具 | 功能说明 | 特色 |
|------|----------|------|
| **课堂互动游戏** | 随机题目与队伍对抗 | 回合倒计时、即时加减分、历史记录 |
...
- 🌐 多语言能力升级：新增西班牙语（Español）和法语（Français）。
- 🎮 新增课堂互动游戏页面：支持随机题目、随机队伍、倒计时、即时计分。
- ♻️ PWA 缓存策略优化：关键资源（HTML/CSS/JS/JSON）改为网络优先，降低用户看到旧版本的概率。
- 📢 公告内容已重写为本次版本说明，并清理旧公告内容。
...
- `HTML/CSS/JS/JSON`：网络优先（优先获取最新版本，离线时回退缓存）
- 图片与字体资源：缓存优先并后台更新
- Service Worker 版本：`2.1.0`
