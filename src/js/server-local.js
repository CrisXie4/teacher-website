...
// HTTP API 路由（用于 Vercel 等不支持 WebSocket 的平台）
...
app.post('/api/join-room', async (req, res) => {
    try {
        ...
        const joined = await joinRoom(roomCode, viewerId);
            ...
            try {
                ...
                if (dbRoom && dbRoom.status === 'active') {
                    res.json({
                        ...
                        room: {
                            roomCode: roomCode,
                            viewerCount: dbRoom.viewer_count || 0,
                            isActive: true,
...
// WebRTC 信令 API（用于交换 offer/answer/ice-candidate）
app.post('/api/signal', async (req, res) => {
    try {
        const { type, roomCode, data, fromId, toId } = req.body;

        // 在实际应用中，这里应该将信令消息存储到数据库或缓存中
        // 供其他客户端轮询获取
        // 这里只是返回成功状态
