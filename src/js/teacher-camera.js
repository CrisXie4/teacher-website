(function() {
    ...
    function applyLanguage() {
        ...
        // 更新按钮文本
        if (startCameraButton) startCameraButton.textContent = lang.start_camera;
        if (createRoomButton) createRoomButton.textContent = lang.create_room;
        if (connectButton) connectButton.textContent = lang.connect_screen;
        if (disconnectButton) disconnectButton.textContent = lang.disconnect;

        // 更新语言切换按钮
        if (languageSwitcher) languageSwitcher.textContent = currentLanguage === 'zh' ? 'English' : '中文';
    ...
    function toggleTheme() {
        ...
        const lang = languages[currentLanguage];
        if (document.body.classList.contains('dark-mode')) {
            if (themeSwitcher) themeSwitcher.textContent = lang.theme_light;
            localStorage.setItem('theme', 'dark');
        } else {
            if (themeSwitcher) themeSwitcher.textContent = lang.theme_dark;
